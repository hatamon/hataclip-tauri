...
`#run` 付きで `{{sh:}}` が無ければ、本文全体を PowerShell として実行する。`#file` は本文をパスと見てファイル内容を貼る（`#run` が優先）。`#type` は `Ctrl+V` ではなく 1 文字ずつ送る。`#once` は貼って成功したらその行を消す。
...
`.` が覚えるのは `dd`・`p` / `P`・`t` / `T`・`m`・`S`・`+` / `-` だけ。移動やヤンク、貼り付けは覚えない。アプリを終了すると忘れる。
