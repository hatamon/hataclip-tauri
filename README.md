...
| パイプ | 左から `\|` でつなぐ（`:sh dir \| sh sort \| quote "x " \| clip`）。最初の `:sh` は stdin なし。次の `:sh` は左の結果を stdin に受け取る。`"` の中の `\|` では切らない。行き先は末尾の1つ。`clip` はクリップボード（前面には貼らない。`> clip` も同じ）。`add` は一覧へ1件（改行は残す。`Ctrl+4` と同じ自動タグ。クリップボードは触らない）。`set a` は変数へ残す。`open` は URL かパスなら開く。`show` はヘルプ画面に出す。行き先が無ければ前面へ貼る。先頭の `.` は選択行（`V` は改行つなぎ。展開しない）。先頭の `clip` はクリップボードを読む |
