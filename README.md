...
| `:quote` | 各行先頭に付ける。未指定は `> `。空白も含めて指定するときは `:quote "* "`。すでに同じ印ならそのまま。履歴は触らない。`:quote | clip` 可（`> clip` も同じ） |
