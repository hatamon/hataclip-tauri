...
- `Ctrl+7` ドロップダウンを表示
- `Ctrl+Shift+H` 前面で選んだ `#foo` / `{{date}}` / `:sh dir` / `:echo 2+3` を置き換えて貼る。`#foo` はいまの一覧で先に出たそのタグの行。無ければ何もしない。一覧は出さない
...
| `Tab` / `Shift+Tab` | 通常はよく使うタグで絞り込みを切り替える。検索中は一覧へ（絞りは残る）。`#` の候補があるときはタグ補完 |
...
`:` の代わりに空白でも同じ。`{{var a}}` `{{env USERPROFILE}}` `{{date %Y%m%d}}` `{{sh echo hi}}`。エクスプローラーの名前変更では `:` が使えない。
