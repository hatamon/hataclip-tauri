...
- `j` / `k` 移動（端で止まる）、`Enter` 貼り付けて閉じる、`Ctrl+Enter` 貼り付けて残す、`Ctrl+C` コピー、`Esc` / `Ctrl+[` 閉じる
- `Ctrl+矢印` ウィンドウ移動
- `Ctrl+Shift+←/→` 幅を縮める・広げる、`Ctrl+Shift+↑/↓` 高さを縮める・広げる
- `/` 検索（IME で日本語変換してから絞る。`tokyo` では `東京` に当たらない）
- 検索中は `Ctrl+N` / `Ctrl+P`（または ↑↓）で移動
- `dd` 削除、`yy` / `Y` ヤンク、`p` 下に戻す、`P` 上に戻す
- `e` その場編集（`Ctrl+Enter` 保存）
