...
#url #path  登録時の自動タグ。動きは目印だけ",
    ...
    (
        "edit",
        "e その場編集（Ctrl+Enter 保存、Esc 取り消し）。E nvim（無ければ $EDITOR、それも無ければメモ帳）。o 空行を作って編集。S 改行で分割。J は V 中なら選んだ行を改行で 1 行に（タグは和集合、ピンはどれかにあれば残す）。c すぐ下に複製。:s/old/new 本文の置換。:sort は V 中なら本文の順。. は dd p P t T gp S J c :s :sort + - を繰り返す。",
    ),
    (
        "sh",
