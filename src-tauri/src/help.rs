...
例: hello を e で world にすると、本文は world。
...
変わるのはこの画面。引数なしは目次。各行の :help 名前 で、その操作だけを出す。j / k でスクロール。
...
貼らない。履歴もクリップボードも触らない。保存しない。終了で消える。| showerror という段にはしない。Esc で閉じる。一覧の検索が 0 件、:s/old/ で選択が空になるのは失敗にしない。CLI には出ない。Ubuntu も同じ。
