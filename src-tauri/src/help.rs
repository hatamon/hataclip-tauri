...
#url #path  登録時の自動タグ。動きは目印だけ",
    ...
    (
        "sh",
        "#run を自分で付けた行だけコマンドを実行する。{{sh: コマンド}} はその場の標準出力に置き換わる。#run 付きで {{sh:}} が無ければ本文全体がコマンド。貼る前に出力を出して Enter で貼る。Esc は中止。:sh dir はその場実行して貼る（一覧で打つ最初の :sh は stdin なし。Ctrl+Shift+H の複数行で1行目なら、2行目以降を stdin に受け取る）。:.!sh xxx はカレント行（V なら改行つなぎ）を stdin に流す。:!!sh xxx は各行を stdin に流して本文を書き換える（貼らない）。式が残るので g: でもう一度。失敗したら本文はそのまま。:@ は直前の :sh / :.!sh のスクリプトをもう一度（stdin はいまの選択）。失敗したら貼らない／書き換えない。| clip でクリップボードへ（貼らない）。| で左からつなぐ。次の :sh は左の結果を stdin に受け取る。",
    ),
    (
        "template",
...
const ENTRIES: &[(&str, &str)] = &[
    ...
    ("sh", "打つ: :sh dir。変わるのは前面（標準出力を貼る）。履歴は触らない。一覧で打つ最初の :sh は stdin なし。Ctrl+Shift+H の複数行で1行目なら、2行目以降を stdin に受け取る。失敗したら貼らない。:.!sh は選択行を stdin に。:!!sh は本文を書き換え、式が残る。Ubuntu でもシェルは動く。例: :sh echo hi で前面は hi。"),
    ("echo", "打つ: :echo 2+3。変わるのは前面（計算結果を貼る）。履歴は触らない。* と / が先。() と :set の変数が使える。計算できなければ何もしない。1行なら 5。複数行で1行目が :echo だけなら何もしない（下の行は残る）。例: :echo 3+4 で前面は 7。:echo 2+3 の次が notes なら置き換えない。"),
    ...
    ("s", "打つ: :s/old/new。変わるのは選択行の本文。履歴のその行だけ。前面には貼らない。正規表現は使わない。old が空なら何もしない。u で戻せる。例: hello に :s/ell/ipp で hippo。段 :sel | s/old/new は前面の old を全部 new にして貼る。履歴は触らない。old が無くても残りを貼る。結果が空でも選択は空になる。Ctrl+Shift+H で選択の1行目が :s/old/new なら、2行目以降を置換して選択全体を置き換える。"),
