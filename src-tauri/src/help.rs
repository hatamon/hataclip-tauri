...
:      help  sh  !!  @  echo  export  import  quote  filter  type  format  raw  join  open  s/  sort  clear  dedup  map  unmap  mapleader  set  n  settings  tags\n\
\n\
...
const ENTRIES: &[(&str, &str)] = &[
    ...
    ("sel", "打つ: :sel | upper のようにパイプの段。変わるのはそのパイプの結果（行き先が無ければ前面）。履歴は触らない。一覧では sel は前面の選択、. は一覧の行。Ctrl+Shift+H の複数行ではどちらも2行目以降。Windows は Ctrl+C の 200ms 後を読んで、すぐクリップボードを戻す。空なら何もしない。Ubuntu では履歴の行にある sel は実行しない。Ctrl+Shift+H の複数行では選択はもう読んであるので sel があっても実行し、結果はクリップボードに置く。例: 前面の hello に :sel | upper で前面は HELLO。"),
    ...
    ("sh", "打つ: :sh dir。変わるのは前面（標準出力を貼る）。履歴は触らない。一覧で打つ最初の :sh は stdin なし。Ctrl+Shift+H の1行で後ろに段があれば stdin なしで実行して選択を置き換える。シェルの | は引用符の中。括らない段が読めなければ何もしない。複数行で1行目なら、2行目以降を stdin に受け取る。失敗したら貼らない。:.!sh は選択行を stdin に。:!!sh は本文を書き換え、式が残る。Ubuntu でもシェルは動く。例: :sh echo hi で前面は hi。"),
    ...
    ("filter", "打つ: :filter \".txt\" または :filter not \".txt\"。変わるのは前面（含む行だけ、not は含まない行）。履歴は触らない。正規表現は使わない。大文字と小文字は区別する。括らなければ空白の後ろ全部。\\\" は \"、\\t はタブ、\\n は改行。針が not なら :filter \"not\"。1行も残らない、引数が空、流れが空なら何もしない。選択は空にしない。例: a.txt と b.rs に :filter \".txt\" で前面は a.txt。:sh dir | filter \".txt\" も同じ。Ctrl+Shift+H の1行は sh か echo が先にあるときだけ。:filter だけの1行は何もしない。"),
    ...
    ("s", "打つ: :s/old/new。変わるのは前面へ貼る文字。履歴は触らない。選択行の old を全部 new にする。正規表現は使わない。old が空なら何もしない。old が無くても残りを貼る。結果が空でも選択は空になる。V は改行つなぎ。u では戻らない。本文を直すのは e。例: 選択行 hello に :s/ell/ipp で前面は hippo。段 :sel | s/old/new は前面の選択を置換して貼る。Ctrl+Shift+H で1行目が :s/old/new なら2行目以降を置換して選択全体を置き換える。1行だけは何もしない。"),
