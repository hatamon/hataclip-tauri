...
const ENTRIES: &[(&str, &str)] = &[
    ...
    ("log", "打つ: :log または :log notes.log。変わるのはそのファイル（選択行を末尾へ追記）。前面には貼らない。履歴は触らない。未指定の出力先は変数 defaultLogFileName。未定義か空、選択が空、展開できない、書けない、なら何もしない。:log path はそのファイル。V は改行つなぎ。例: :set defaultLogFileName=notes.log のあと hello を :log で notes.log に hello。"),
