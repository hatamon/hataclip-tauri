...
#url #path  登録時の自動タグ。動きは目印だけ",
    ...
    (
        "edit",
        "e その場編集（Ctrl+Enter 保存、Esc 取り消し）。E nvim（無ければ $EDITOR、それも無ければメモ帳）。o はいまの行の下、O は上に空行を作って編集。V なら o は範囲の最後の下、O は最初の上。起点がピンなら追加した行もピンで、そのすぐ隣。ピンでなければピンにしない。一覧が空なら先頭の1件。検索中もその行の隣に残る。S 改行で分割。J は V 中なら選んだ行を改行で 1 行に（タグは和集合、ピンはどれかにあれば残す）。c すぐ下に複製。:!!sh は各行をコマンドで書き換える。:sort は V 中なら本文の順。. は dd p P t T gp ga S J c :!! :sort g~ g: + - を繰り返す。g: は行に残した式をもう一度実行する。式が無ければ何もしない。sel はいまの前面の選択、. はいまの本文。失敗したら本文はそのまま。:from はその式だけを編集する（e は本文）。| add と :!!sh でできた行に式が残る。g~ は #grab の1行目と2行目を入れ替える。2行目が無ければ何もしない。3行目以降はそのまま。u で戻せる。",
    ),
    (
        "sh",
...
編集   dd 削除  yy ヤンク  p P 置く  u Ctrl+R 取り消し  . 繰り返し\n\
       o O 空行  e 編集  E nvim  S 分割  J まとめ  c 複製  t T タグ  gp ピン  ga #app  + -\n\
...
const ENTRIES: &[(&str, &str)] = &[
    ...
    ("sel", "打つ: :sel | upper のようにパイプの段。変わるのはそのパイプの結果（行き先が無ければ前面）。履歴は触らない。一覧では sel は前面の選択、. は一覧の行。Ctrl+8 の複数行ではどちらも2行目以降。Windows は Ctrl+C の 200ms 後を読む。クリップボードは戻さない。空なら何もしない。Ubuntu では履歴の行にある sel は実行しない。Ctrl+8 の複数行では選択はもう読んであるので sel があっても実行し、結果はクリップボードに置く。例: 前面の hello に :sel | upper で前面は HELLO。"),
